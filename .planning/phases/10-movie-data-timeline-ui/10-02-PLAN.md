...
Use the same Dialog, Input, Button component imports as save-clip-dialog.tsx.
  ...
  <verify>Run `npm run build` to check for TypeScript/import errors. Navigate to http://localhost:3000/movie in browser -- page should render with empty state or movie list. Click "Create Movie" -- dialog should appear, create movie, and navigate to /movie/[id]. Click "Movie" link in sidebar -- should navigate to /movie and show active state. Click a movie card -- should navigate to /movie/[id] editor page.</verify>
  <done>Movie list page at /movie shows user's movies in a grid with loading, empty, and populated states. Create Movie dialog creates a movie and navigates to its editor page. Clicking a movie card navigates to /movie/[id]. Sidebar "Movie" link navigates to /movie with active highlight.</done>
</task>
