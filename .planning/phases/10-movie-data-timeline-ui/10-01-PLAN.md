...
must_haves:
  ...
  key_links:
    ...
    - from: "convex/movies.ts"
      to: "convex/movies.ts"
      via: "addScene enforces uniform fps"
      pattern: "fps.*match|fps.*mismatch"
---
...
1. `create` (mutation) - Args: `{ name: v.string() }`. Auth required. Insert movie with empty scenes array, totalDurationInFrames: 0, fps: 30 (default — updated when first scene is added if clip has different fps), current timestamps. Return the new movie ID.
