...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/components/creation/variation-stack.tsx"
      ...
      min_lines: 15
  # NOTE: These patterns already exist in creation-modal.tsx (Phase 24).
  # Task 1 only creates stubs to resolve imports; Task 2 verifies behaviors work.
  key_links:
    - from: "src/components/creation/creation-modal.tsx"
      to: "@radix-ui/react-dialog"
      via: "onEscapeKeyDown and onPointerDownOutside props (EXISTING)"
      pattern: "onEscapeKeyDown.*handleClose"
    - from: "src/components/creation/creation-modal.tsx"
      to: "next/navigation router"
      via: "router.replace() for arrow navigation (EXISTING)"
...
**Type imports:** Use `import { Doc } from "../../../../convex/_generated/dataModel"` and define `type Generation = Doc<"generations">` for the Generation type in each file.

**Export pattern:** Use NAMED exports (not default) to match how CreationModal imports them:
...
1. **creation-detail-panel.tsx**
   ...
   - Render: A placeholder div with text "Details panel - Phase 26"
...
2. **creation-edit-bar.tsx**
   ...
   - Make it focusable so arrow key guarding can be tested (CRITICAL for NAV-05)
...
- Use NAMED exports (not default) matching creation-modal.tsx imports
