...
must_haves:
  truths:
    ...
    - "User can render a single clip to MP4 from the create page via a Render button"
  artifacts:
...
<task type="auto">
  <name>Task 3: Create ClipRenderButton and wire into create page</name>
  <files>
    src/components/render/clip-render-button.tsx
    src/app/(app)/create/create-page-client.tsx
  </files>
  <action>
  Create `src/components/render/clip-render-button.tsx`:
  - "use client" directive
  - Similar pattern to MovieRenderButton but for individual clips
  - Props: `clipId: string`, `disabled?: boolean`
  - Use `useAction(api.triggerRender.startClipRender)` to trigger render
  - Track render status (simple isRendering state + toast feedback, since clip renders don't have a getByClip query -- use optimistic UI: button shows "Rendering..." while action is in progress, then success/error toast)
  - Three states: idle (show "Render MP4" button), rendering (show spinner), complete (toast success -- the render result will show in the existing render progress/download mechanism on the create page if it exists, or simply confirm the render was triggered)
  - On click: call `startClipRender({ clipId: clipId as any })`, toast on success/error

  Modify `src/app/(app)/create/create-page-client.tsx`:
  - Import ClipRenderButton
  - In the render controls area (near the existing ExportButtons or Save button), add `<ClipRenderButton clipId={clipId} disabled={!lastGeneration} />` when a clip is loaded (clipId is present)
  - The button renders a single clip to MP4 via Lambda using DynamicCode composition
  - Only show when there is a valid clip/generation to render
  </action>
  <verify>
  Run `npm run build` -- build succeeds. Navigate to create page, load a clip from library. Verify "Render MP4" button appears. Clicking it should trigger the startClipRender action (may fail if Lambda is not configured, but the action call should work).
  </verify>
  <done>
  ClipRenderButton exists on create page, triggers startClipRender action for individual DynamicCode clips. OUT-03 (single clip MP4 export) is now covered.
  </done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <what-built>
  Complete movie render and export flow + single clip rendering:
  ...
  4. Single clip render to MP4 from create page
  All wired into their respective pages.
  </what-built>
  <how-to-verify>
  ...
  4. Click "Render MP4" on movie page -- verify progress appears (Note: actual Lambda rendering requires AWS setup; the button click and API call should work, but render may fail if Lambda is not configured)
  ...
  8. Navigate to create page, load a clip -- verify "Render MP4" button appears for single clips
  </how-to-verify>
...
7. ClipRenderButton appears on create page when a clip is loaded, triggers startClipRender action
</verification>
...
- User can click Render MP4 on create page for a single clip and the startClipRender action is triggered
</success_criteria>
