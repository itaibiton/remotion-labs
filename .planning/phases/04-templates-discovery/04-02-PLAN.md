...
must_haves:
  truths:
    ...
    - "Selected template shows context banner on create page"
  artifacts:
...
Purpose: Completes the user flow - users can browse templates, preview them, select one, and land on the create page with template context pre-filled.
...
<task type="auto">
  ...
  <action>
1. Update `src/app/create/page.tsx`:
   - Add import: `import { useSearchParams } from "next/navigation"`
   - Add import: `import { getTemplateById } from "@/lib/templates"`
   - Add import: `import Link from "next/link"` (if not already)

2. In CreateContent component:
   - Add: `const searchParams = useSearchParams()`
   - Add: `const templateId = searchParams.get("template")`
   - Add: `const selectedTemplate = templateId ? getTemplateById(templateId) : null`
...
Note: For v1, template selection shows context but prompt still drives generation. Future enhancement could pre-fill props directly.
  </action>
  <verify>
    `npm run dev` - visit http://localhost:3000/create?template=bold-announcement, template banner shows
  </verify>
  <done>
    Create page shows template context when ?template= param present, "Change template" links back to /templates
  </done>
</task>
...
<verification>
1. `npm run build` - Next.js build succeeds
...
- User can select template and land on /create with context banner
