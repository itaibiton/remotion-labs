...
5. Add a `useEffect` to populate state when clip data loads:
   ...
   useEffect(() => {
     if (clipData && clipId) {
       ...
       setChatMessages([]);
       // Note: Setting lastGeneration triggers the preview Player to update
       // because Player's inputProps depend on lastGeneration state.
       // skipValidation=true is correct here because the clip's code was
       // already validated when it was saved — no need to re-validate.
     }
   }, [clipData, clipId]);
   ```

   Place this useEffect AFTER the existing `storeUser` useEffect and BEFORE the `handleGenerate` callback. It should only run when `clipData` changes (i.e., when the query resolves). Setting `lastGeneration` will cause the preview Player to re-render with the loaded clip's code since the Player's `inputProps` derive from this state.
